...
namespace PUPiMed
{
    public partial class UCFacultyTab : UserControl
    {
        ...
        {
            InitializeComponent();
            strQuery = "SELECT * FROM vwfacultylogs WHERE `Last Name` LIKE '" + txtFilter.Text + "%' OR `Middle Name` LIKE '" + txtFilter.Text + "%' OR `First Name` LIKE '" + txtFilter.Text + "%';";
            updateTable();
        }
        ...
        private void getValues()
        {
            ...
            try
            {
                if (LoginNew.conn != null && (LoginNew.conn.State == ConnectionState.Closed))
                    LoginNew.conn.Open();
                using (MySqlCommand cmd = new MySqlCommand("SELECT strPatiCode,intPatiType,strPatiLastName,strPatiFirstName,strPatiMiddleName,datPatiDOB,intPatiGender,fltPatiHeight,fltPatiWeight,strPatiContact,strFacICollCode from dbmedicalclinic.tblpatient left join dbmedicalclinic.tblfacultyinfo ON strpaticode = strfaciPatiCode where strpaticode = '" + code + "';", LoginNew.conn))
                {
