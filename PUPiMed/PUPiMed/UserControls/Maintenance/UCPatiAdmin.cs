...
namespace PUPiMed
{
    public partial class UCAdminTab : UserControl
    {
        public string strQuery;
        FormAddPatient formpatient;
        FormAddPatient form;

        public UCAdminTab()
        {
            InitializeComponent();
            strQuery = "SELECT * FROM vwadminlogs WHERE `Last Name` LIKE '" + txtFilter.Text + "%' OR `Middle Name` LIKE '" + txtFilter.Text + "%' OR `First Name` LIKE '" + txtFilter.Text + "%';";
            updateTable();
        }
        ...
        private void getValues()
        {
            ...
            try
            {
                if (LoginNew.conn != null && (LoginNew.conn.State == ConnectionState.Closed))
                    LoginNew.conn.Open();
                using (MySqlCommand cmd = new MySqlCommand("SELECT strPatiCode,intPatiType,strPatiLastName,strPatiFirstName,strPatiMiddleName,datPatiDOB,intPatiGender,fltPatiHeight,fltPatiWeight,strPatiContact,strAdmIDeptCode from dbmedicalclinic.tblpatient left join dbmedicalclinic.tbladmininfo ON strpaticode = stradmiPatiCode where strpaticode = '" + code + "';", LoginNew.conn))
                {
                    using (MySqlDataReader dataReader = cmd.ExecuteReader())
                    {
                        ...
                        if (dataReader.Read())
                        {
                            ...
                            college = dataReader.GetString(10);
                        }
                    }
                }
            }
